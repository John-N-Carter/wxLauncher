...
##        self._mainSizer.Destroy()
    ...
    def SetPageColour(self, page, colour):
        """
        Sets the tab colour individually.

        :param `page`: an integer specifying the page index;
        :param `colour`: a valid :class:`wx.Colour` object or any typemap supported by wxWidgets/wxPython
         to generate a colour (i.e., a hex string, a colour name, a 3 or 4 integer tuple). You can
         pass ``None`` or :class:`NullColour` to return to the default page colour.
        """
        #~ print(f'Setting colour of page {page} to {colour}')
...
##        self.DoSetSelection(page)
    ...
    def SetPageColour(self, page, colour):
        ...
        :param `colour`: a valid :class:`wx.Colour` object or any typemap supported by wxWidgets/wxPython
         ...
         pass ``None`` or :class:`NullColour` to return to the default page colour.
        """
        #~ print(f'Realy seting colour ({colour}) for page {page}')

        if page < len(self._pagesInfoVec):
            self._pagesInfoVec[page].SetColour(colour)
            self.Refresh()
